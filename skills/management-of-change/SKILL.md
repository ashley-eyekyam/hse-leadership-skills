...
description: 'Produces a Management of Change (MoC) package: the technical basis for
  the change, a risk assessment of the change, temporary-change expiry, the Pre-Start-Up
  Safety Review (PSSR) as a hard pre-start-up gate, and document/training updates
  (OSHA 1910.119(l), ISO 45001 8.1.3). No start-up authorization is issued until the
  PSSR checklist passes. Decision-support only; a competent person must review the
  output.'
license: Apache-2.0
metadata:
  author: eyekyam
...
A consultant-grade skill that produces a Management of Change (MoC) package and enforces the Pre-Start-Up Safety Review (PSSR) as a **hard pre-start-up gate**: the technical basis for the change, a risk assessment of the change (`risk_matrix` initial vs residual), temporary-change expiry, the PSSR checklist, and the document/training updates (OSHA 1910.119(l), ISO 45001 8.1.3). **No start-up authorization is issued until the PSSR checklist passes** — that is the load-bearing runtime gate. Controls are HoC-ranked; actions are tracked with `smart_actions`.

## When to use this skill

Use this skill to prepare or review an MoC for a **named, specific change** — e.g. "MoC for replacing the relief valve on V-101", "risk-assess this temporary bypass", "build the PSSR checklist before start-up". It surfaces for O&G and process users. It refuses a vague "a change" — the intake forces the exact change, and it will not authorize start-up until the PSSR gate passes.
...
| Process (any) | ../../knowledge-base/standards/psm.md (KB-STD-PSM — MoC element (l) + PSSR element (i)) + ../../knowledge-base/regulatory/us-osha.md (KB-REG-US-OSHA — 1910.119(l)) |
