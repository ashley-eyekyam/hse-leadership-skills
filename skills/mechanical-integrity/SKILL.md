...
description: 'Structures a mechanical / asset-integrity program: equipment criticality
  ranking, inspection/test/preventive-maintenance (ITPM) schedules, deficiency management,
  and integrity-operating-windows framing. Produces a criticality-ranked equipment
  register with a hierarchy-of-controls deficiency-remediation plan and SMART actions
  tracked to closure. Decision-support only; a competent person must review the output.'
license: Apache-2.0
metadata:
  author: eyekyam
  version: '1.0'
...
A consultant-grade skill that structures a mechanical / asset-integrity program: equipment criticality ranking (`risk_matrix`), inspection/test/preventive-maintenance (ITPM) schedules, deficiency management, and integrity-operating-windows (IOW) framing. It produces a criticality-ranked equipment register with a HoC-ranked deficiency-remediation plan (`controls`) and SMART actions tracked to closure (`smart_actions`). It implements PSM element (j) mechanical integrity.

## When to use this skill

Use this skill to build or review a mechanical-integrity program for a **named equipment population / unit** — e.g. "rank our pressure-vessel criticality", "set ITPM intervals for the Crude Unit", "manage these integrity deficiencies". It surfaces for O&G and process users. If the request is vague, the intake forces the specific equipment population and the integrity basis before any register is produced.
...
| Process (any) | ../../knowledge-base/standards/psm.md (KB-STD-PSM — element (j) mechanical integrity) |
