---
name: psm-program-manager
description: 'Structures and tracks an OSHA PSM 14-element process-safety program
  (29 CFR 1910.119): per-element status (compliant / gap / overdue), evidence, gap-risk
  band, and owners — the program backbone. Produces a 14-element status matrix with
  a hierarchy-of-controls remediation plan and SMART actions tracked to closure. Decision-support
...
A consultant-grade skill that structures and tracks an OSHA PSM 14-element process-safety program (29 CFR 1910.119). It builds the 14-element status matrix — per element: status (compliant / gap / overdue), evidence, gap-risk band (`risk_matrix`), and owner — HoC-ranks the gap-remediation controls with `controls`, and tracks the remediation plan to closure with `smart_actions`. It is the program backbone the other process skills implement element-by-element.

## When to use this skill

Use this skill to set up or review a PSM program for a **named facility / covered process** — e.g. "build our PSM 14-element status matrix", "where are our PSM gaps", "track our PHA / MoC / mechanical-integrity element status". It surfaces for O&G and process users. If the request is vague, the intake forces the specific facility and the elements in scope before any matrix is produced.
...
| Process (any) | ../../knowledge-base/standards/psm.md (KB-STD-PSM — the 14-element framework) + ../../knowledge-base/regulatory/us-osha.md (KB-REG-US-OSHA — the US statutory hook 29 CFR 1910.119) |
