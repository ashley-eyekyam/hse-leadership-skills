# De-identification Checklist (A5) — rail-safety-management-system (moderate tier)
...
> cannot be waived. This is the **moderate** de-id tier: a rail SMS is a circulated,
> assurance-facing document, so named **safety-critical role-holders**, **COSS**
> (controller of site safety), and **Sentinel competence-card numbers** must become
> role labels in any distributed copy.

The canonical A5 steps, with the rail reinforcement (CONV-7) that this skill needs:
...
2. **PSEUDONYMIZE BY DEFAULT** to stable **role labels** ("Accountable Duty-Holder",
   "Safety-Critical Role A", "COSS (role)", "Driver A"). Keep the re-identification key in
   a SEPARATE access-controlled artifact; **never** embed the key or any name↔label
   mapping (and never embed a Sentinel number) in the SMS.
...
4. **WARN BEFORE WIDE DISTRIBUTION** — the SMS circulates to ORR, ECM partners, and
...
5. **MINIMIZE & LIMIT PURPOSE** — use only the personal data the SMS needs; keep
   fitness-for-duty / occupational-health detail out of the circulated SMS (it lives in the
   separate confidential competence/medical record).
