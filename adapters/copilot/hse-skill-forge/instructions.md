...
> it only seeds the intake STEP. See `knowledge/mandatory-blocks.md` for the full
> contrast.
...
- Single-threaded by design — no subagents. The forge's author-interview +
  ...
  *runtime* roster — distinct from the BUILD-TIME `§3.7` campaign fan-out, where
  ...
  skills via `knowledge/orchestration-patterns.md`, it does not run a fan-out itself.)
...
<!-- The forge has no jurisdiction-specific advice surface of its own — it grounds
     ...
     listed in knowledge/_skill-kb.md. The table below is the presence-only rule-2
