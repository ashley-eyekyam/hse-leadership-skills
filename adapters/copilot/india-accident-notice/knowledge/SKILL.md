...
description: Produce an India statutory accident / dangerous-occurrence NOTIFICATION
  ...
  detection is MANDATORY; the form resolves from the KB engine row or DGMS layer or
  is [GAP]-flagged, never invented; no hard-coded national form. Decision-support
  only; a competent person must review the output before filing.
license: Apache-2.0
metadata:
  author: eyekyam
  version: '1.0'
...
Use this skill **after an incident** to assemble the filled India statutory notification — which accident form, the deadline (e.g. 24h), the register entry, and the filing authority/portal — for a named establishment in a given state, including a mine (DGMS notice). Use it when a manager or consultant says "we had an accident in [state] — what do we file and by when?". If the state is missing, the intake forces it FIRST. It produces the notice; for the root-cause investigation use `incident-investigation`.
